...
| # | 項目 | 内容 | 規模感 |
|---|---|---|---|
| 1 | **ACL / 権限の Linux↔Windows 相互運用** (主目的達成、残項目あり) | 設計の正は [docs/permission-interop.ja.md](permission-interop.ja.md) / 図は [permission-interop-diagram.html](permission-interop-diagram.html)。PGFS は名前ベース ACL ストアで、POSIX mode が正準・Windows は投影ビュー。名前ハンドリング (正規化 [NameNormalizer](../src/core/src/Utility/NameNormalizer.cs) / ドメイン除去 / principal マッピング / `user.win.attrs` JSON / ReadOnly を st_mode で表現) と ACL 本体 (正準モデル [PgfsAcl](../src/core/src/Models/PgfsAcl.cs)、Windows `GetFileSecurity`/`SetFileSecurity` 投影、Linux POSIX ACL [PosixAcl](../src/core/src/Models/PosixAcl.cs) `system.posix_acl_access` ⇄ mode + `user.pgfs_acl`) は実装 + 回帰済み。**残**: named ACL の厳密 enforce (要件が出てから)、`system.posix_acl_default` の Windows 継承変換、cross-OS 往復の自動テスト | 中 |
...
| # | 項目 | 内容 | 規模感 |
|---|---|---|---|
| 7 | **コーディング規約 v4 への追従** | `else` 句 ~20 + 三項演算子 ~20、計約 40 箇所。書き換えパターンは `FirstList.cs` (switch 式・タプル分解・Try* 抽出) と [Api.cs](../src/core/src/Api/Api.cs) (Logger ガードのブレース化) 参照。詳細は [coding-style.ja.md](coding-style.ja.md) | 各ファイル単位で小 |
...
- **Mount オプション `-o`**: `-o key=val,flag,...` を [ConfigLoader.ParseDashOOptions](../src/core/src/Config/ConfigLoader.cs) で分類 (FUSE passthrough / 受理して無視 + `x-` 接頭辞 / pgfs 設定 / 未知=Warning / 非対応マウント操作=明示 Warning)。詳細は [Mount.ja.md §マウントオプション](Mount.ja.md)。Assign 側 `-o` は将来検討。
...
- **ヘルプ自動生成**: 手書き `ShowHelp` を廃止し、[HelpText.Build](../src/core/src/Config/HelpText.cs) が [Schema.AllFields](../src/core/src/Config/Schema.cs) を walk して `--help` を組み立てる。ツール別の出し分けは [Field.AppliesTo](../src/core/src/Config/Field.cs)。
- **`Field` self-check**: [ConfigLoader.Resolve](../src/core/src/Config/ConfigLoader.cs) が触れた Field を記録し、[ConfigLoader.UnresolvedFields](../src/core/src/Config/ConfigLoader.cs) が `Schema.AllFields` との差分 (= 配線忘れ) を起動時 Warning に積む。
- **CLI 引数の二重 parse 解消**: mount/assign の `BuildRootConfig` が lite+full の 2 Loader で CLI/TOML を二度パースしていたのを、同一 Loader 使い回し + [ConfigLoader.WithStore](../src/core/src/Config/ConfigLoader.cs) による Phase 3 後付けに統一 (CLI/TOML パースは 1 回)。
...
- **設定モデル**: 旧 `*Settings.cs` ツリーを削除、`Pgfs.Core.Config` (`Field<T>` + `ConfigLoader` + `ConfigStore`) に統一。詳細は [architecture.ja.md §Config](architecture.ja.md)。
...
- **Notify (LISTEN/NOTIFY)**: cross-client 変更通知 ([src/core/src/Api/NotifyChannel.cs](../src/core/src/Api/NotifyChannel.cs))、`database.notify_enabled` opt-in。**→ 多 client 運用の前提**
- **接続 retry**: [Retry.cs](../src/core/src/Utility/Retry.cs) で transient 失敗 (PG 再起動 / ネット瞬断) を指数バックオフ再試行。**→ 運用上の前提**
...
- **ログ出力先**: `logging.output = "daily:~/pgfs/log/pgfs-*.log"` とローテーションを [LogSink](../src/core/src/Logging/LogSink.cs) + [RotatingFileSink](../src/core/src/Logging/RotatingFileSink.cs) で `Logger.Output` に配線。
