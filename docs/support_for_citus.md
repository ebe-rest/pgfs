...
1. **`--citus` x `--tablespace` (!= pg_default) cannot be combined** ([ValidateConfigCombinations](../src/mkfs/src/Initializer.cs)). Citus shard placement fails without a tablespace of the same name on every worker, so it is rejected pre-flight. This also removes the need for a tablespace branch in the "report existing state" path.
