...
| 2 | **xattr byte transparency** | currently via Base64 inside JSONB ([Api.cs](../src/lib/src/Api/Api.cs) `EncodeXattrValue`/`DecodeXattrValue`). A branch for arbitrary binary xattrs (e.g. SELinux) + added test cases. Also a prerequisite for the ACL-over-xattr delivery path (POSIX ACLs delivered over the `system.posix_acl_access` xattr — see [permission-interop.md](permission-interop.md)) | medium (Schema change) |
| 3 | **ACL / permission Linux↔Windows interop** | design source of truth is [docs/permission-interop.md](permission-interop.md) (diagram: [permission-interop-diagram.html](permission-interop-diagram.html)). PGFS is a name-based ACL store; POSIX mode is canonical and Windows is a projection view. The immediate name-handling items are implemented and regression-tested (name normalization via [NameNormalizer](../src/lib/src/Utility/NameNormalizer.cs); domain stripping; principal mapping; `user.win.attrs` JSON; ReadOnly via st_mode). The ACL itself is implemented: a canonical model ([PgfsAcl](../src/lib/src/Models/PgfsAcl.cs)), the Windows `GetFileSecurity` read projection, the `SetFileSecurity` reverse projection + owner=group routing, and the Linux POSIX ACL path ([PosixAcl](../src/lib/src/Models/PosixAcl.cs), `system.posix_acl_access` ⇄ mode + `user.pgfs_acl`). Regression: Windows 26/26, Linux 35/35, race 4/4. Deferred: strict named-ACL enforcement (deferred until requirements arrive), `system.posix_acl_default` → Windows inheritance translation, and automated cross-OS round-trip tests | medium |
...
| 8 | **Windows e2e on multi-node Citus** | the cross-client locking verification ([tests/citus/race_multinode.sh](../tests/citus/race_multinode.sh)) ran only the Linux 35/35. A slot to run the Windows 26/26 via docker Citus is not yet created |
...
- **Linux↔Windows permission/ACL interop**: PGFS is a name-based ACL store with POSIX mode canonical and Windows as a projection view. Name handling (normalization, domain stripping, principal mapping, `user.win.attrs`, ReadOnly via st_mode) plus the full ACL path (canonical `PgfsAcl`, Windows `GetFileSecurity`/`SetFileSecurity` projection, Linux POSIX ACL over `system.posix_acl_access`) are implemented and regression-tested (Windows 26/26, Linux 35/35, race 4/4). Details in [permission-interop.md](permission-interop.md). **→ premise for feature #2 / #3**
- **Citus support** (distribution + cross-client locking): Large Object → bytea, `mkfs --citus [--worker ...]`, `pgfs_lock` + `SELECT FOR UPDATE` locking, Linux e2e 35/35 + concurrent write/mkdir race + reasonable lock accumulation on multi-node Citus. Details in [support_for_citus.md](support_for_citus.md), verification in [tests/citus/race_multinode.sh](../tests/citus/race_multinode.sh). **→ premise for performance #14 / #15**
