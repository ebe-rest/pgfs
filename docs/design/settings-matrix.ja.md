...
| `purge` | **`--purge`** | ❌ | ❌ | false | bool | mkfs `Initializer.TeardownAsync` | **v0.2.1 で追加**。消して終わる (`-f` の接続先の DB + Citus の全 worker の同名 DB)。`--clean` と同時は exit 2。短縮形なし |
| `yes` | **`--yes`** `-y` | ❌ | ❌ | false | bool | mkfs `Initializer.ConfirmTeardownAsync` | **v0.2.1 で追加**。`--clean` / `--purge` の確認を省く。非対話で無ければ exit 3 |
| `now` | **`--now`** | ❌ | ❌ | false | bool | mkfs `Initializer.WaitUntilNoConnectionsAsync` | **v0.2.1 で追加**。接続中のもの (生きているマウント / 他の接続) を待たずに切る |
...
| `mount.self_uname` | `--self-uname` | ✅ | ❌ | `""` (未設定) | string | `UserResolver` / `WindowsUserResolver` 構築 (NextMount) | v0.2.1〜。**自分の名乗り (補完)**: マウントを動かすプロセス自身のユーザーが作ったものにこの名前を付ける (名前が引けても引けなくても上書き)。他のユーザーの要求には使わない。この名前は自分の uid / SID として見せる。旧 `mount.fallback_uname` (廃止) の役割の一部 |
| `mount.self_gname` | `--self-gname` | ✅ | ❌ | `""` (未設定) | string | 同上 | v0.2.1〜。自分の名乗りの group 版。設定があれば、自分が作ったものの gname をこれにする (Windows の親からの継承 / Linux の primary group より優先) |
...
| キー | CLI | TOML | DB | 既定 | 型 | 参照タイミング | 備考 |
|---|---|---|---|---|---|---|---|
| `file_system.version` | `--fs-version` (v0.2.1 で `--version` から改名) | ❌ | ✅ | `1.0.0` | string | (現状コード上の参照なし) | mkfs 時凍結用。将来の互換性チェック余地 |
| `file_system.volume_label` | `--volume-label` | ❌ | ✅ | `pgfs` | string | `assign.pgfs` の `GetVolumeInformation` | Windows のドライブ名 |
| `file_system.unknown_name` | (なし・実質固定) | ❌ | ✅ | `(unknown)` | string | mkfs が作るときに投入 / mount・assign 起動時 (Format) | v0.2.1〜。作った人の名前が分からないとき (Windows で要求元が取れない / 自分以外の SID・uid が名前に引けない) に DB へ書く名前。uname と gname の両方。OS の画面ではそのホストに無い名前なので overflowuid / 既知の SID として見える。いずれ mkfs / pgfsctl で指定できるようにする予定 |
| `file_system.root_access` | `--root-access` | ❌ | ❌ | `owner` | enum (`owner` / `everyone`) | mkfs `Initializer.InsertRootInodeAsync` (root を**新規作成するときだけ**) | v0.2.1〜。`owner` = `root:root 0755` / `everyone` = `root:root 1777` (sticky)。mkfs 専用のアクション (SaveTo=None)。既存 root には効かず Warning |
...
| `database.tablespace` | `--tablespace` `--tablespace-name` | ❌ | ❌ | `pg_default` | string | mkfs `Initializer.EnsureTablespaceAsync` (coordinator + 全 worker) | **Citus でもカスタム可**。per-table 句を廃し `CREATE DATABASE WITH TABLESPACE` 継承。**DB 権威化** (設定ファイルで書き換えさせない fs 識別情報。生成 toml 非出力)。**v0.2.1 から DB に保存しない** (作るときだけの指示。既存 FS では DB の実体から取る)。**既存の DB で `--citus` を渡しても、Citus でなければ Citus にならない** (Warning・v0.2.1) |
| `database.tablespace_path` | `--tablespace-path` | ❌ | ❌ | `""` | string | mkfs `Initializer.EnsureTablespaceAsync` | 空文字なら新規作成しない。指定時、`app.plperlu` 許可なら plperlu auto-mkdir (postgres 所有 0700)。**DB 権威化**。**v0.2.1 から DB に保存しない** (作るときだけの指示。既存 FS では DB の実体から取る) |
...
| `database.notify_enabled` | `--notify` `--notify-enabled` / 否定 `--no-notify` | ✅ | ❌ | **`true`** (v0.2.1〜。v0.2.0 までは `false`) | bool | 起動時にデータ変更通知の送受信を選択 (制御 LISTEN は常時起動) | 他クライアント変更通知 (LISTEN/NOTIFY)。複数マウントでは事実上必須なので既定を on にした。1 マウントだけの運用で LISTEN 接続 1 本と `pg_notify` を省きたいときは `--no-notify`。mkfs に渡すと配布用 toml に書かれる。詳細 [docs/Mount.ja.md](../Mount.ja.md) / [docs/Assign.ja.md](../Assign.ja.md) |
| `database.citus` | **`--citus`** | ❌ | ❌ | `false` | bool | mkfs `Initializer` が PGFS テーブルを Citus 分散登録するか | **DB 権威化** (SaveTo=File→Db)。「この FS は Citus 化済み」を後追い確認する bool。mount/assign は条件分岐に使わない。詳細 [docs/support_for_citus.ja.md](support_for_citus.ja.md)。**v0.2.1 から DB に保存しない** (作るときだけの指示。既存 FS では DB の実体から取る) |
| `database.workers` | `-w` `--worker` `--workers` | ✅ | ❌ | 空リスト | List&lt;string&gt; (Field) | mkfs の Citus worker bootstrap | `host[:port],...`。mount/assign の live 変更対象ではない。**DB を作るときだけ効く** (v0.2.1。既存の DB に渡すと Warning で、worker 側にロール / tablespace も作らない) |
| `database.shard_count` | `--shard-count` | ❌ | ❌ | `0` | int | mkfs が `create_distributed_table` 前に `citus.shard_count` をこの値に設定 (`0` = クラスタ既定に従う) | mkfs 専用。セッション GUC なので同じ DB を共有する他アプリに影響しない。詳細 [support_for_citus.ja.md](support_for_citus.ja.md)。**v0.2.1 から DB に保存しない** (作るときだけの指示。既存 FS では DB の実体から取る) |
| `database.shard_replication_factor` | `--shard-replication-factor`, `--rf` | ❌ | ❌ | `0` | int | shard 1 つを何ノードに置くか (`0` = クラスタ既定に従う) | mkfs 専用。**ストレージ冗長の選択**で、排他制御 (`{prefix}lock` = 非分散) には影響しない。**v0.2.1 から DB に保存しない** (作るときだけの指示。既存 FS では DB の実体から取る) |
...
| `app.enforce_permissions` | (なし・`pgfsctl config set`) | ❌ | ✅ | `true` | bool | assign の `CreateFile` / `MoveFile` ([FileSystem.Access.cs](../../src/dokan/src/FileSystem.Access.cs)) | **v0.2.1 で追加**。Windows (Dokan) で POSIX の権限 (mode + 正準 ACL) を判定するか。Dokan は SD で判定しないので assign が自前で見る。`false` は v0.2.0 と同じ「判定しない」。**Linux には効かない** (`default_permissions` のカーネル判定)。mkfs は行を書かない (無ければ既定 `true`)。設計 [permission-interop.ja.md §Windows の判定](permission-interop.ja.md) |
...
| **NextMount** (既定) | 再マウントで反映 | `mount.mount_point` / `mount.max_write` / `mount.self_uname` / `mount.self_gname` / `database.connection` / `database.schema` / `database.prefix` / `database.notify_enabled` / `app.plperlu` (作るときだけの指示 `database.tablespace` / `tablespace_path` / `citus` / `workers` / `shard_count` / `shard_replication_factor` は保存しないので reload の対象外・v0.2.1〜) |
...
- **Db + Live** (`audit.enabled` / `app.statfs` / `app.enforce_permissions`): `pgfs_settings` 書込み (永続) + `set` NOTIFY (即時 live)。
...
- `mount.fallback_uname` / `mount.fallback_gname` は **v0.2.1 で廃止** (指定すると専用の Warning)。このホストに無い名前は OS の overflowuid / overflowgid (Windows は `ANONYMOUS LOGON`) で見せ、名前が分からないときは `file_system.unknown_name` を DB に書く
