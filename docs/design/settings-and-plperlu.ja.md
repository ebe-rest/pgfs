...
> plperlu × statfs の挙動マトリクス、Citus カスタム tablespace の auto-mkdir、
> mkfs 生成 toml に残す配布用コメントはここに書く。**個々の設定項目の現在値は本書では追わない**。
>
> **隣接する doc とその担当範囲**:
>
> | doc | そちらに書くもの |
...
`--citus`+カスタム tablespace + plperlu auto-mkdir (dir を postgres 所有 0700 で作成) / mount サニティ)。
さらに **本番 pgsql_server の mkfs で Chunk 1 を実機確認** (app.statfs(当時 pgfs.statfs)/database.citus/app.plperlu/file_system サイズ系が
`pgfs_settings` に保存、生成 toml に配布コメント・サイズ系非出力)。

## 動機

1. **fs 全体で 1 値であるべき設定が toml にしか無い** → 2 クライアントが違う値で繋ぐと事故る。
   実機 mkfs 後の `pgfs.toml` に `[file_system] cluster_size / default_chunk_size / max_file_size` が
   ...
   statfs.mode) は DB 権威で正しい。→ サイズ系も DB 権威にする。
2. **plperlu (untrusted) を使ってよいか**の上位ゲートが欲しい。今は `--statfs nominal` が事実上の
   「plperlu 使わない」だったが、tablespace auto-mkdir (#23) でも plperlu を使うため、判断を 1 か所に
   切り出す。
3. **「この FS は Citus か」を後から DB で確認したい** (今は toml の `database.citus` のみ)。
...
- canonical / 正の別名 (`--plperlu` / `--allow-plperlu`) にマッチ → true をセットし、**直後の引数が `true`/`false`
  リテラルならそれを値として消費**、そうでなければ bare=true のまま (`--allow-plperlu` は値を取らない運用でよいが、
  実装簡略化のため「正の別名は値を飲める」で統一して可)。
- negated 別名 (`--deny-plperlu`) にマッチ → false をセット (値は飲まない)。
- 既存の他 bool フラグ (`--foreground` 等) は正の別名のみなので挙動不変 (bare=true、リテラルが続かなければ値消費なし)。

### (B) `statfs.mode` → `app.statfs` (スコープ移動)

アプリ挙動なので `app` スコープへ (`app.plperlu` と同 scope)。値は従来どおり `auto|require|nominal`、SaveTo=Db 不変。

- 旧: `Scope="statfs", Key="mode"` (`pgfs_settings` の `statfs / mode`)
- 新: `Scope="app", Key="statfs"` (`pgfs_settings` の `app / statfs`)。C# 参照は `Schema.Statfs.Mode` のまま。
- CLI は `--statfs` / `--statfs-mode` 不変。
- (検討経緯: 当初 `pgfs.statfs` に移したが、`pgfs` scope が statfs 単独になるため `app.statfs` に再変更。`pgfs` scope は廃止)。

### (C) `database.citus` を DB 権威化 (名前変更なし)

「この FS は Citus 化されている」かの bool を DB に持つ。当初 `pgfs.citus` への改名も検討したが
**改名は取り消し** (既存 `database.citus` のまま)。SaveTo だけ変更:

- 旧: `Scope="database", Key="citus", SaveTo=File`
- 新: `Scope="database", Key="citus", **SaveTo=Db**`、CLI `--citus` 不変、AppliesTo=Mkfs。
...
- mkfs の [WriteTomlFile/AddField](../../src/mkfs/src/Program.cs) は元々 **SaveTo=File のみ**書き出すので、サイズ系を
  Db にすれば**自動的に生成 toml から消える** (追加コード不要。明示の `AddField` 呼び出し 3 行は no-op になるので除去)。
- [pgfs.toml.example](../../pgfs.toml.example) と各 doc の toml サンプルからサイズ系・statfs・citus を**外す**
  (「toml で設定できる」と誤解させない)。
- ただし**手書きで toml に書いた場合は現状仕様どおり読んでその通り動く** (TOML>DB)。事故っても書いた人の責任。
...
|---|---|---|
| `auto` | やってみて、ダメなら nominal 相当で続行 (現状通り) | nominal に倒す |
| `nominal` | ストアドを作らない (現状通り) | ストアドを作らない (現状通り) |
| `require` | やってみて、ダメならエラー | **エラー** (plperlu 不許可で require は矛盾) |

## tablespace auto-mkdir + Citus 制約撤廃 (#23)
...
DO LANGUAGE plperlu $PL$
  use File::Path qw(make_path);   # 再帰作成 OK (合意済み)
  make_path($ENV{PGFS_TS_DIR});   # ※パスの渡し方は実装時に確定 (DO は引数を取らないので
...
- plperlu は postgres OS ユーザで動くので、作成 dir は **postgres 所有 0700** = CREATE TABLESPACE の要求条件に一致
  (今まで運用者が手で `sudo mkdir/chown/chmod` していたのが不要に)。
- Citus は `run_command_on_all_nodes` で全ノードに mkdir DO を流す (dir はノードローカル)。1 ノード Citus は
  pgsql_server 1 台で成立。多ノードは親 dir が postgres から書ける前提 (書けなければ make_path も失敗 → 明示エラー)。
...
- df 相乗効果: `pgfs_fs_free` が `pg_tablespace_location` で実体 dir を引くので、`--statfs require` 時の `df` が
  **tablespace 実体 (例 /database/tbs/pgfs) の実空き**を返す (今は pg_default 強制で data_directory)。

## 移行

開発段階なので in-place 移行はしない。スキーマ/設定キー変更後は `mkfs --clean` で作り直す前提。
`pgfs_settings` の行は (scope,key) が変わる (statfs/mode → app/statfs、database.citus は SaveTo=Db 化のみ、
file_system サイズ系が新たに DB 行として増える)。

## 触る場所

- [Schema.cs](../../src/core/src/Config/Schema.cs): `app` nested class 新設 (`Plperlu`)、`Statfs`→`Pgfs` に統合
  (`Pgfs.Statfs` / `Pgfs.Citus`)、`FileSystem` サイズ系の SaveTo 変更、`Database.Citus` 移設。
...
- 接続文字列の Password は既に `[database].connection` に平文で出ているので情報漏洩は増えないが、コメント側は
  [DescribeProvided](../../src/core/src/Config/ConfigLoader.cs) と同様に **Password をマスク**して出すのが無難 (実装時に確定)。

## 決定事項 (open questions の解決)
