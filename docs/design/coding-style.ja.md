...
> 条件分岐ルール (`else` / ternary 禁止と多分岐の `switch` 化)、ガードを呼ばれる側へ押し込むリズム、
> コメント方針、ホットパスのロガーガードはここに書く。
>
> **隣接する doc とその担当範囲**:
>
> | doc | そちらに書くもの |
...
> **規約**
> 1. **文字列 ↔ 値の変換 (parse / format) を持つ enum は、その変換を enum と同じファイルに置く。**
> 2. **クラスで包むかどうかは問わない。** `static class Xxx { enum Enum }` でも、裸の `enum Xxx` +
>    同ファイルの `static class XxxText` でもよい。**包みの有無は規約の対象外**。
> 3. **変換を持たない enum は裸で宣言する。**

**「包めば振る舞いの置き場ができる」は成立しない**、というのが現物から得た結論である。
`src/core` + `src/fuse` の enum 10 個 (うち 2 個は libfuse の P/Invoke 由来で対象外) を数えると:

| 分類 | 現物 | 実態 |
|---|---|---|
| 包み + 振る舞いあり | [`Level.Enum`](../../src/core/src/Logging/Level.cs) | **1 個だけ**。`Parse(string)` / `Parse(int)` を持つ |
| 包み + 振る舞い**なし** | [`SettingLoggingCycle.Enum`](../../src/core/src/Models/SettingLoggingCycle.cs) (12 行) / [`SettingLoggingKind.Enum`](../../src/core/src/Models/SettingLoggingKind.cs) (14 行) | **空のクラスで包んだだけ**。中身は enum 宣言のみ |
| 裸 | `ReloadPolicy` / `SaveTarget` / `Tool` / `CoalesceResult` / `PendingState` / `PendingAddResult` | 振る舞いなし |

**決定的なのは `SettingLoggingCycle`**: **包んであるのに parse も format も
[`Field.cs`](../../src/core/src/Config/Field.cs) にある** (`"hourly" => ...Enum.Hourly` と
`...Enum.Hourly => "hourly"` が別々の switch で並ぶ)。包みがあっても、そこに置かなければ散る。
逆に `Level` が散っていないのは包んだからではなく、**`Parse` を 1 箇所に置いたから**である。

**包みには 2 重のコストがある**:

1. 呼び名が 1 階層深くなる。`Field.cs` には
   `Pgfs.Core.Models.SettingLoggingCycle.Enum.Hourly` (名前空間込みで 4 階層) が 15 行以上並ぶ。
2. **それを隠すためのエイリアスを書くことになる。** `Level` は
   `public const Enum Trace = Enum.Trace;` を **8 本**持つ — `Level.Enum.Trace` と書きたくないので
   `Level.Trace` を作り直している。**enum が裸の `Level` なら最初から `Level.Trace` である。**

したがって統一すべきは**構文ではなく振る舞いの所有**である。

**数え方の注意**: 「その enum を参照しているファイル数」で散らばりを測らないこと。**宣言と判断が混ざる**。
`SaveTarget` は 13 ファイルに出るが、ほとんどは `Schema.cs` と各 `*Config.cs` の**宣言**であり、
散らばりではない。**値を見て分岐している箇所だけ**を数える。
...
if (OperatingSystem.IsWindows()) {
    Console.Error.WriteLine("mount.pgfs は Windows では使えません。assign.pgfs (DokanNet 版) を使ってください。");
    return 1;
}

// ログ複数行 + 脱出 — ログはいくつあってもカウント外
if (!Pgfs.Fuse.Fuse.CheckDependencies()) {
    Console.Error.WriteLine("FUSE の依存が見つかりません:");
...
| 6. ternary 禁止 | △ (**ツールに書かせない**ことはできる。書かれたものの検出は不可) |

**この 6 つのうち機械強制が効くものは無い。** 2026-09-21 に実測して確認した。**レビューで担保する。**
(**条件分岐ルールの外では、ブレース必須がビルドで効く** — 下の注)。
将来必要になれば `Pgfs.Analyzers` 的な小さなプロジェクトを追加して Roslyn アナライザで強制する手がある。

> **6 について (2026-09-21 に実測で訂正)**: 本表はかつて 6 を「✅ (`.editorconfig` で抑制可能)」と
> 書いていたが、**それは誤りだった**。`.editorconfig` の
> `dotnet_style_prefer_conditional_expression_over_assignment` / `_over_return` は
> **IDE0045 / IDE0046 = 「`if` を ternary にまとめろ」という逆向きのルール**を制御するもので、
> **`false` に倒すとツールが ternary を勧めてこなくなるだけ**である。
> **手書きの ternary は 1 件も報告されない** (`EnforceCodeStyleInBuild=true` を付けて実測、0 件。
> **同じ設定を `true` にすると `if` 文に IDE0046 が出る**ので、アナライザが動いていないのではない)。
> **`.editorconfig` には上記 2 行を入れてある** — 効果は「ツール側から ternary が戻ってこない」ことだけである。
>
> **逆に、機械強制が効くものが 1 つある**: **ブレース必須 (`csharp_prefer_braces = true:warning`) は
> ビルドで検査される。** `src/Directory.Build.props` に
> **`EnforceCodeStyleInBuild = true`** を入れたため (それまでは設定済みなのに**ビルドでは効いていなかった**)。
> **違反すると `IDE0011` が warning で出る。** 有効化のときに `src/fuse` の 18 件
> (`FuseFileInfo.cs` 12 / `FuseMount.cs` 6) を潰してある。
> **ブレースを 1 つ外して警告が出ることまで確認済み**なので、**「0 件」は測定失敗ではない。**
...
過去に整理した実績例 (削除の判断はファイル全体がコメントアウトされている等、明確な場合):
- `Collections/extentsions.cs` / `tests.cs` — 削除
