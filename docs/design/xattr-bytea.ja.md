...
> `{prefix}inode` のスキーマ差分、Api の配列演算への書き換え、in-memory 表現、移行方針、
> 検証すべき往復ケースはここに書く。
>
> **隣接する doc とその担当範囲**:
>
...
現状は `pgfs_inode.xattrs JSONB` に `{ 名前: Base64(値) }` で保持している
...
- inode 行に同梱のまま → 現状の「inode ロード時に全 xattr を一緒に取る」キャッシュ設計を壊さない
...
原子性は現状の JSONB `||`/`-` と同じく **サブクエリ無しの単一 UPDATE 文**で保つ
...
(pgsql_server の `pgfs` DB / docker テスト DB とも再構築)。
...
- 既存 `system.posix_acl_access` (3-3) が引き続き通ること (回帰)
