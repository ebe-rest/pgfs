...
namespace Pgfs.Fuse
{
    ...
    // Partial mirror of libfuse's `struct fuse_config` (fuse.h), up to the only field the pgfs
    // fork sets: `use_ino`. Setting use_ino=1 in the init callback makes the kernel honor the
    // FS-provided st_ino (from GetAttr) instead of synthesizing one — required for hard-link
    // identity. Field order/types match libfuse 3.x; on LP64 this places use_ino at offset 64
    // (verified against 3.14). Trailing fields (readdir_ino, direct_io, ...) are omitted: we only
    // ever set use_ino, via a pointer cast over the native struct, so they don't affect anything.
    // Partial mirror of libfuse's `struct fuse_conn_info` (fuse_common.h), up to the only field the
    // pgfs binding sets: `max_write`. libfuse fills this struct from the kernel's INIT request and
    // passes it to the init callback; raising max_write there makes the kernel send larger WRITE
    // requests (capped by FUSE_MAX_PAGES = 1 MiB), which cuts the number of write transactions.
    // `-o max_write=...` is NOT accepted by libfuse 3 (fuse_new fails), so init is the only hook.
    // All fields are `unsigned int`, so the layout is trivially stable; trailing fields
    // (max_read, max_readahead, capable, want, ...) are omitted because we never touch them.
