...
/// <summary>
/// One entry of the Linux <c>system.posix_acl_access</c> / <c>system.posix_acl_default</c> xattr blob.
/// The layout is <c>struct posix_acl_xattr_entry { __le16 e_tag; __le16 e_perm; __le32 e_id; }</c>.
/// </summary>
...
/// <summary>
/// Encodes and decodes the POSIX ACL xattr blob (<c>struct posix_acl_xattr_header</c> followed by an
/// entry array). Converting between st_mode (the three base classes) and <see cref="PgfsAcl"/> (named
/// entries) is the caller's job (FileSystem), because it needs name-to-id resolution. The source of
/// truth for the design is decisions [5] and [10] in
/// [docs/permission-interop.md](../../../../docs/permission-interop.md).
/// </summary>
public static class PosixAcl
{
	public const uint Version = 2;            // POSIX_ACL_XATTR_VERSION
	public const ushort TagUserObj = 0x01;    // ACL_USER_OBJ (owner; id=undef)
	...
	/// <summary>Splits the xattr blob into entries. Returns null on a version mismatch or a bad size.</summary>
	...
	/// <summary>Builds the xattr blob (header + entries) from a list of entries.</summary>
