...
namespace Pgfs.Fuse
{
    class FuseMount : IFuseMount
    {
        ...
        private unsafe void Init(IntPtr ptr, IntPtr ptr2)
        {
            ...
            // legacy `-o use_ino` is no longer accepted via fuse_new args). We overlay the typed
            // `fuse_config` (see LibFuse.structs.cs) on the native pointer instead of poking a
            // magic byte offset, so the field is named and self-documenting; the e2e hardlink
            // tests catch any libfuse fuse_config layout regression.
            if (ptr2 != IntPtr.Zero)
            {
                ((fuse_config*)ptr2)->use_ino = 1;
            }
            // pgfs: pin the per-request write size (fuse_conn_info.max_write) when configured.
            // pgfs turns each FUSE WRITE into one PG transaction, so this is the write transaction
            // granularity. Measured on Linux 5.14 + libfuse 3.10, libfuse already negotiates up to the
            // kernel cap (FUSE_MAX_PAGES = 1 MiB), so 0 (the default) is normally what you want; this
            // hook exists to lower it or to pin it where the negotiated default is smaller.
            // `-o max_write` is rejected by libfuse 3 (fuse_new fails), so init is the only hook.
            if (ptr != IntPtr.Zero && _mountOptions.MaxWrite > 0)
            {
                ((fuse_conn_info*)ptr)->max_write = (uint)_mountOptions.MaxWrite;
            }
        ...
        {
            // libfuse calls the handlers with `path == NULL` once `fuse_config.nullpath_ok` is set
            // (read, write, flush, release, fsync, readdir, releasedir, fsyncdir, ...). pgfs does
            // not set it yet, so NULL cannot reach here today -- but leaving the dereference
            // unguarded means whoever turns it on gets a crash inside every callback instead of a
            // path they can follow. Hand back an empty span so the caller falls into its existing
            // "no usable path" branch (since handle-context stage B the hot callbacks key off the
            // handle and only touch the path for logging and the fallback lookup).
            if (path == null)
            {
